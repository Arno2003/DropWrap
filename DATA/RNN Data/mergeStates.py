...
# Read the CSV files
df_final_impressions = pd.read_csv('DATA\RNN Data\outputData\final_impressions.csv')
stateDNoMapping_df = pd.read_csv('DATA\Test\Abbreviations\stateDNoMapping.csv')

# Remove "IndiaDistricts" from stateDNoMapping_df
stateDNoMapping_df = stateDNoMapping_df[stateDNoMapping_df["state"] != "IndiaDistricts"]
...
df_final_impressions['state'] = df_final_impressions['DNo'].apply(lambda x: map_dno_to_state(x, stateDNoMapping_df))
